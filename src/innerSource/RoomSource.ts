export const roomSource: string = /* language=JavaScript */ `
	import {$, roomEmitter, getConnections} from "#inner";
...
export const roomInnerSource: string = /* language=JavaScript */ `
    ...
    export const getConnections = ({ready} = {ready: undefined}) => {
        if (ready === undefined) return new Set(connections.values());
        if (ready) return new Set([...connections.values()].filter(con => con.ready));
